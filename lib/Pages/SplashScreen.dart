...
void getSavedDataFromDatabase(BuildContext savedContext) async {
  try {
    ...
    NativeItem? nativeItem = box.get('native_item_key'); // Get the NativeItem object from the box

    if (nativeItem != null) {
      Navigator.of(savedContext).pushReplacementNamed('/home', arguments: nativeItem);

      print('getnativeItem ${nativeItem.toJson()}');
    } else {
      showDialog(
